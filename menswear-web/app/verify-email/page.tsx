  ...
  return (
    <div className="min-h-[80vh] grid place-items-center px-4 py-12 bg-cream">
      <div className="w-full max-w-md rounded-xl border border-sand bg-cream/90 shadow-soft p-6 text-center">
        <h1 className="text-2xl font-semibold text-brownDark mb-2">Verify your email</h1>
        {status === 'verifying' && (
          <p className="text-brown/80">Verifying your email address…</p>
        )}
        {status !== 'verifying' && (
          <p className={`text-sm ${status === 'success' ? 'text-emerald-700' : 'text-brown/80'}`}>{message}</p>
        )}
        <div className="mt-6 flex justify-center gap-3">
          <Link href="/login" className="px-4 py-2 rounded-md bg-brown text-cream hover:opacity-95">Sign in</Link>
          <Link href="/register" className="px-4 py-2 rounded-md border border-brown text-brown hover:bg-sand/60">Register</Link>
        </div>
      </div>
    </div>
  )
