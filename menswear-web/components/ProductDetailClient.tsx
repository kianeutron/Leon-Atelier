        ...
        <div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 md:grid-cols-3">
            {(images && images.length
              ? images.slice(0, 3)
              : ([imageUrl].filter(Boolean) as string[]).slice(0, 3)
            ).map((u, i) => (
              <button
                key={i}
        ...
        <div className="md:sticky md:top-24 lg:top-28">
          <div className="flex items-start justify-between gap-4">
            <motion.h1
              ...
              className="font-display text-3xl md:text-4xl text-brownDark leading-tight"
            >
              {title}
            </motion.h1>
            <button
              ...
              onClick={() =>
                wished
                  ...
                  : add({ productId, title, image: activeImg })
              }
          ...
          {wished && (
            <div className="mt-1 text-xs">
              <Link href="/liked" className="text-brown underline">
                View in Liked
              </Link>
            </div>
          )}
          <div className="mt-4 text-2xl md:text-[28px] text-brownDark">
          ...
          <div className="mt-7">
                  ...
                  <button
                    ...
                    className={`rounded-full border px-4 py-2 text-sm transition backdrop-blur select-none ${
                      active
                        ...
                        : disabled
                          ? 'border-sand/70 bg-cream/60 text-brown/40 cursor-not-allowed'
                          : 'border-sand bg-cream hover:border-brown'
                    }`}
                    aria-pressed={active}
            ...
            <Link
              href="/size-guide"
              className="mt-2 inline-block text-xs text-brown underline underline-offset-4"
            >
              Size guide
            </Link>
          </div>
          ...
          <div className="mt-6 flex items-center gap-4">
            <div className="inline-flex items-center rounded-full border border-sand bg-cream overflow-hidden">
              <button
                ...
                className="px-3 py-2 text-brown hover:bg-sand/50"
              >
                −
              </button>
            ...
            <div className="flex-1">
              ...
              />
              {!size && <p className="mt-2 text-xs text-red-700">Please select a size.</p>}
            </div>
          </div>
          ...
          <div className="mt-8 divide-y divide-sand border-y border-sand bg-cream/50 rounded-lg">
            <details className="group p-4">
              <summary className="cursor-pointer list-none flex items-center justify-between text-brownDark">
                Details <span className="text-brown/60 group-open:rotate-180 transition">⌄</span>
              </summary>
              <div className="mt-3 text-brown/80 text-sm">
                {description ||
                  'Soft hand-feel, tailored fit, and durable construction in warm neutrals.'}
              </div>
            </details>
            <details className="group p-4">
              <summary className="cursor-pointer list-none flex items-center justify-between text-brownDark">
                Shipping & returns{' '}
                <span className="text-brown/60 group-open:rotate-180 transition">⌄</span>
              </summary>
              <div className="mt-3 text-brown/80 text-sm">
                Free shipping over $150. 30-day returns. Free exchanges.
              </div>
            </details>
            <details className="group p-4">
              <summary className="cursor-pointer list-none flex items-center justify-between text-brownDark">
                Care <span className="text-brown/60 group-open:rotate-180 transition">⌄</span>
              </summary>
              <div className="mt-3 text-brown/80 text-sm">
                Dry clean recommended. Steam to refresh. Avoid direct heat.
              </div>
            </details>
          </div>
