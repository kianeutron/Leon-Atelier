  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0"
        style={{ background: 'radial-gradient(800px 400px at 0% 0%, rgba(43,43,43,0.35), rgba(43,43,43,0.12) 40%, transparent 65%)' }} />
      <div aria-hidden className="pointer-events-none absolute inset-0 bg-[radial-gradient(1400px_600px_at_50%_120%,rgba(0,0,0,0.06),transparent)]" />
          ...
          <div className="relative h-full w-full">
            {slides.map((s, i) => (
              <motion.img
                ...
                decoding="async"
                initial={false}
                animate={{ opacity: i === index ? 1 : 0, scale: i === index ? 1 : 1.02, x: i === index ? 0 : -6 }}
                transition={{ duration: 0.6, ease: 'easeOut' }}
                style={{ WebkitMaskImage: 'linear-gradient(to left, black 78%, transparent 100%)', maskImage: 'linear-gradient(to left, black 78%, transparent 100%)' }}
              />
            ))}
          </div>
      ...
      <svg className="absolute z-[1] left-[-220px] xl:left-[-280px] -top-10 w-[900px] h-[900px] opacity-70 hidden xl:block pointer-events-none" viewBox="0 0 800 800" aria-hidden>
        <defs>
        ...
        <circle cx="280" cy="320" r="300" fill="none" stroke="#6A4B3A" strokeOpacity="0.25" strokeWidth="26" />
        <circle cx="280" cy="320" r="230" fill="none" stroke="#C96F55" strokeOpacity="0.3" strokeWidth="18" />
      </svg>
      ...
      <div className="mx-auto max-w-6xl px-4 py-16 md:py-24 grid grid-cols-1 xl:grid-cols-[1.1fr_1fr] gap-8 items-center">
        ...
        <div className="relative z-10">
          ...
          >
            Textured layers and tailored silhouettes in warm cream and deep umber, built for crisp evenings and city light.
          </motion.p>
          <motion.div
            initial={{opacity:0,y:20}}
            animate={{opacity:1,y:0}}
          ...
          >
            <a className="btn-accent" href="#shop">Explore More ▷</a>
            <a className="px-6 py-3 rounded-md bg-brownDark text-cream hover:bg-brown transition border border-sand/50 shadow-sm" href="/products">Shop All</a>
          </motion.div>
          {/* Socials */}
          <motion.div
          ...
          >
            <a href="#" aria-label="Instagram" className="hover:text-brownDark"><Instagram size={18}/></a>
            <a href="#" aria-label="Twitter" className="hover:text-brownDark"><Twitter size={18}/></a>
            <a href="#" aria-label="Email" className="hover:text-brownDark"><Mail size={18}/></a>
          </motion.div>
