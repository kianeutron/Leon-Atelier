...
export function CartDrawer() {
  ...
  const freeShipPct = isFreeShip ? 100 : Math.min(100, Math.round((effectiveSubtotal / freeShipThreshold) * 100))

  function applyPromo() {
    setPromoMsg(null); setPromoError(null)
    const code = promo.trim().toUpperCase()
    if (!code) { setPromoError('Enter a promo code'); return }
    if (appliedCode) { setPromoError('A code is already applied'); return }
    // Demo validation rules
    if (code === 'WELCOME10') {
      const tenPct = Math.floor(subtotal * 0.10)
      const capped = Math.min(tenPct, 5000) // max $50
      if (capped <= 0) { setPromoError('Cart is empty'); return }
      setDiscountCents(capped)
      setAppliedCode(code)
      setPromoMsg('WELCOME10 applied: 10% off (up to $50)')
    } else if (code === 'SAVE20') {
      if (subtotal < 2000) { setPromoError('Minimum subtotal $20 required'); return }
          ...
          >
            ...
            <div className="p-4 md:p-5 border-b border-sand flex items-center justify-between">
              <div>
                ...
                <p className="text-xs md:text-sm text-brown/70">{items.length} item{items.length !== 1 ? 's' : ''}</p>
              </div>
              <button onClick={closeCart} aria-label="Close cart" className="rounded-md p-2 hover:bg-sand/60 text-brown">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="px-4 md:px-5 py-3 border-b border-sand bg-cream/60">
              <div className="flex items-center gap-2 text-brown/80 text-sm"><Truck size={16}/> {isFreeShip ? (
                <span>You’ve unlocked free shipping</span>
              ) : effectiveSubtotal < freeShipThreshold ? (
                <span>{formatMoney(freeShipThreshold - effectiveSubtotal)} away from free shipping</span>
              ) : (
                <span>You’ve unlocked free shipping</span>
              )}
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-5 space-y-4">
              {items.length === 0 ? (
                ...
                items.map(i => (
                  <div key={i.id} className="grid grid-cols-[64px_1fr_auto] gap-3 items-center rounded-xl border border-sand bg-cream/70 p-3">
                    {/* thumb */}
                    <div className="h-16 w-16 rounded-lg overflow-hidden bg-sand/50 flex items-center justify-center text-brown/60">
                      ...
                      { (i as any).image || (i as any).img ? (
                        <img src={(i as any).image ?? (i as any).img} alt={i.title} className="h-full w-full object-cover" />
                      ) : (
                    ...
                    <div className="text-right">
                      ...
                      <button onClick={() => remove(i.id)} className="mt-1 inline-flex items-center gap-1 text-xs text-brown/70 hover:text-brownDark">
            ...
            <div className="px-4 md:px-5 pb-3">
              <div className="relative">
                <div className="absolute left-3 top-1/2 -translate-y-1/2 text-brown/60"><Tag size={16}/></div>
                <input value={promo} disabled={!!appliedCode} onChange={(e)=>setPromo(e.target.value)} placeholder="Promo code" className="w-full rounded-md border border-sand bg-cream pl-9 pr-32 py-2.5 text-sm text-brown outline-none focus:ring-2 focus:ring-brown/30 disabled:opacity-60" />
                {appliedCode ? (
                  <button onClick={removePromo} className="absolute right-1.5 top-1.5 rounded-md bg-sand px-3 py-1.5 text-sm text-brown hover:bg-sand/70">Remove</button>
                ) : (
                  <button onClick={applyPromo} className="absolute right-1.5 top-1.5 rounded-md bg-brown text-cream px-3 py-1.5 text-sm hover:bg-brownDark">Apply</button>
                )}
              </div>
            ...
            <div className="sticky bottom-0 z-10 border-t border-sand bg-cream px-4 md:px-5 py-4">
              ...
              {isFreeShip && <p className="mt-1 text-xs text-emerald-700">Free shipping will be applied</p>}
              <div className="mt-2 flex items-center justify-between text-brownDark">
                <span>Total</span>
                <span>{formatMoney(effectiveSubtotal)}</span>
              </div>
              <p className="mt-1 text-xs text-brown/60">Taxes and shipping calculated at checkout.</p>
              <div className="mt-4 flex gap-3">
                <button onClick={clear} className="px-4 py-2 border border-brown text-brown rounded-md hover:bg-sand/60">Clear</button>
                <Link href="/register" onClick={closeCart} className="flex-1 px-4 py-2 bg-brown text-cream rounded-md hover:bg-brownDark text-center">Checkout</Link>
              </div>
            </div>
          </motion.aside>
        </>
