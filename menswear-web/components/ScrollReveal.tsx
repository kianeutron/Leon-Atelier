...
export function ScrollReveal({ children, className, delay = 0, y = 16, margin = "-100px 0px -100px 0px" }: Props) {
...
export function FadeUp({ children, delay = 0, y = 12 }: { children: ReactNode; delay?: number; y?: number }) {
  const prefersReduced = useReducedMotion()
  return (
    <motion.div
      variants={prefersReduced ? undefined : { hidden: { opacity: 0, y }, show: { opacity: 1, y: 0, transition: { duration: 0.5, ease: 'easeOut', delay } } }}
      initial={prefersReduced ? undefined : "hidden"}
