...
public class AuthController : ControllerBase
{
    ...
    {
        try { return await HandleResendVerificationAsync(dto); }
        catch (Exception ex) { return Problem(title: "Resend verification failed", detail: ex.Message, statusCode: 500); }
    }

    [HttpGet("dev/verification-link")]
    public async Task<IActionResult> GetVerificationLink([FromQuery] string email)
    {
        try { return await HandleGetVerificationLinkAsync(email); }
        catch (Exception ex) { return Problem(title: "Dev link failed", detail: ex.Message, statusCode: 500); }
    }
    ...
    {
        try { return await HandleRegisterAsync(dto); }
        catch (Npgsql.PostgresException pgx) when (pgx.SqlState == "23505") { return Conflict(new { error = "Email already registered" }); }
        catch (Exception ex) { return Problem(title: "Register failed", detail: ex.Message, statusCode: 500); }
    }

    [HttpPost("login")]
    public async Task<IActionResult> Login([FromBody] LoginDto dto)
    {
        try { return await HandleLoginAsync(dto); }
        catch (Exception ex) { return Problem(title: "Login failed", detail: ex.Message, statusCode: 500); }
    }
    ...
    {
        try { return await HandleMeAsync(); }
        catch (Exception ex) { return Problem(title: "Me failed", detail: ex.Message, statusCode: 500); }
    }

    [HttpPost("verify-email")]
    public async Task<IActionResult> VerifyEmail([FromBody] VerifyEmailDto dto)
    {
        try { return await HandleVerifyEmailAsync(dto); }
        catch (Exception ex) { return Problem(title: "Verify email failed", detail: ex.Message, statusCode: 500); }
    }
    ...
    private async Task<IActionResult> HandleResendVerificationAsync(ResendVerificationDto dto)
    {
        ...
        if (user is null || user.IsEmailVerified) return Ok(new { ok = true });
        ...
        try { if (_mailer != null) await _mailer.SendAsync(user.Email, subject, html); } catch { }
        Console.WriteLine($"[EmailVerification] Resent verify link to {user.Email}: {verifyUrl}");
        return Ok(new { ok = true });
    }

    private async Task<IActionResult> HandleGetVerificationLinkAsync(string email)
    {
    ...
    private async Task<IActionResult> HandleRegisterAsync(RegisterDto dto)
    {
    ...
    private async Task<IActionResult> HandleMeAsync()
    {
    ...
    private async Task<IActionResult> HandleVerifyEmailAsync(VerifyEmailDto dto)
    {
        var token = dto.Token;
